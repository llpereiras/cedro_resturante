class RestaurantesController < ApplicationController
  ...
  def index
    ...
    where = params[:pesquisar].to_s.empty? ? nil : {nome: params[:pesquisar]}
    @restaurantes = Restaurante.where(where)
  end
  ...
  def create
    ...
    respond_to do |format|
      if @restaurante.save
        format.html { redirect_to restaurantes_path, notice: 'Restaurante foi criado.' }
  ...
  def update
    respond_to do |format|
      if @restaurante.update(restaurante_params)
        format.html { redirect_to restaurantes_path, notice: 'Restaurante foi atualizado.' }
  ...
  def destroy
    @restaurante.destroy
    respond_to do |format|
      format.html { redirect_to restaurantes_path, notice: 'Restaurante foi excluído.' }
  ...
  private
    ...
    def restaurante_params
      params.require(:restaurante).permit(:nome, :endereco)
    end
end
